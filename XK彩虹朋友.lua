...
local win = lib:Window("XKHUB彩虹朋友",Color3.fromRGB(0, 255, 0), Enum.KeyCode.E)

local tab = win:Tab("主要")
local tab2 = win:Tab("透视")
...
tab2:Toggle("物品透视", false, function(bool)
    if bool then
...
tab2:Toggle("怪物透视", false, function(bool)
    if bool then
        local runService = game:GetService("RunService")
        event = runService.RenderStepped:Connect(function()
