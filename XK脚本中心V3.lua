...
Tab21:Toggle("自动刷钱", "AL", false, function(AM)
    _G.autoFarm = AM
...
Tab21:Toggle("自动刷钱", "AM", false, function(AM)
...
local creds = window:Tab("忍者元素大亨", "6035145364")
    local about = creds:section("通用内容", true)
    
    about:Toggle("箱子透视","Valkiry",false,function(state)
        if state then
            _G.Tree2ESPInstances = {}
            local esptable = {doors = {}}
            ...
            local function monitorTree2()
                for _, instance in pairs(workspace:GetDescendants()) do
                    if instance:IsA("Model") and instance.Name == "BalloonCrate" then
                        createBillboard(instance, "箱子", Color3.new(255, 0, 0)) -- Change color as needed
                    end
                end
            ...
            local function monitorTree2()
                for _, instance in pairs(workspace:GetDescendants()) do
                    if instance:IsA("Model") and instance.Name == "Door" then
                        createBillboard(instance, "门", Color3.new(255, 0, 0)) -- Change color as needed
                    end
                end
    ...
    about:Toggle("钱与物品透视","Valkiry",false,function(state)
    物品if state then
            _G.nahInstances = {}
            local esptable = {nah = {}}
    ...
    about:Toggle("飞行脚本","Valkiry",false,function(state)
    isFlying = val
        if isFlying then
